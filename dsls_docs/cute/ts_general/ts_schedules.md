...
function instantiates a `Schedule` that you pass to
`Task(schedule=...)`.
...
- `start`, `end`, `step` – the loop bounds. `start` and `step`
  ...
  [Dynamic Domain](ts_schedules.md#dynamic-domain).
...
[Resources](ts_resources.md#rts-resources) for how resources declare these variables.
...
cadence (see [Validation](ts_validation.md#rts-validation) for the implications).
...
[Forwarding Contextual Information](ts_schedules.md#forwarding-contextual-information)) – rather than a runtime loop in the
captured schedule.
