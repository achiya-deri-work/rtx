...
Constructing a `TaskManager` runs `print_and_verify()` immediately.
With the default `verbose=True` it prints, in order:

1. the **schedule table** (one column per task, simulated step by step);
2. the **register budget** summary;
3. the **SMEM / TMEM usage** summary and the allocator usage report;
4. the **exhaustive checker** output, when it is enabled.
...
- **call-idx** – the call index of the step’s schedule slot (distinguishes
  ...
  argument (see [Compile-Time Work Arguments](ts_resources.md#rts-constexpr-work-args)).
...
[Validation](ts_validation.md#rts-validation).
...
raise an error (see [Validation](ts_validation.md#rts-validation)).
...
See [Allocators](ts_allocators.md#rts-allocators) for how the layout and alias groups are built.
...
kernels for faster compilation. See [Validation](ts_validation.md#rts-validation) for the full checker
behavior.
