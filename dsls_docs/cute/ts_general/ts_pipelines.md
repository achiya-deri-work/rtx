...
brackets its reads with `wait` / `release`. A `PipelineConfig`
...
or races on hardware. See [Validation](ts_validation.md#rts-validation).

## Supported pipeline types

| Type | Producer -> Consumer | Typical use | Factory |
| --- | --- | --- | --- |
| `AsyncAsync` | async threads -> async threads | SMEM filled from registers / load from GMEM and read by async threads (no TMA, no MMA). | `create_async_async_pipeline_cfg` |
| `TmaAsync` | TMA -> async threads | SMEM tile loaded by TMA and consumed by async threads (copy, epilogue staging). | `create_tma_async_pipeline_cfg` |
| `TmaUmma` | TMA -> UMMA | A/B operand SMEM loaded by TMA and consumed by a `tcgen05` MMA (the GEMM mainloop). | `create_tma_umma_pipeline_cfg` |
| `UmmaAsync` | UMMA -> async threads | TMEM accumulator produced by MMA and consumed by the async threads (TMEM-to-register). | `create_umma_async_pipeline_cfg` |
| `AsyncUmma` | async threads -> UMMA | SMEM filled by an async / `cp.async` producer and consumed by a `tcgen05` MMA. | `create_async_umma_pipeline_cfg` |
| `UmmaUmma` | UMMA / UTCCP -> UMMA | A UTCCP producer feeding a UMMA consumer (both `tcgen05`). | `create_umma_umma_pipeline_cfg` |
| `ClcFetchAsync` | CLC fetch -> async threads | The id queue of a CLC dynamic-persistent scheduler. | `create_clc_fetch_async_pipeline_cfg` |

## Common configuration fields

Every factory returns a `PipelineConfig`. The simplest fields are:
...
shape; see [Validation](ts_validation.md#rts-validation).
...
[Scheduling Patterns](ts_patterns.md#rts-patterns) for the complete setup.
...
Do not build `PipelineConfig` directly; call a factory and attach the
