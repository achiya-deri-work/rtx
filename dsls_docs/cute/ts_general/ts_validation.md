...
- **Leader-routed producer completions** - When a TMA producer routes its
  ...
  using `num_bytes_per_warp_per_cta` (see [Scheduling Patterns](ts_patterns.md#rts-patterns)).
- **Interleaved pipelines** - A resource with `interleave_stride > 1` must
  ...
  [Scheduling Patterns](ts_patterns.md#rts-patterns).
...
- **Split-consumer signaling** - In a *split-consumer* pattern the `wait` and
  ...
  actually present; setting it otherwise is rejected. See [Scheduling Patterns](ts_patterns.md#rts-patterns).
...
- **PipelineGroup consistency** - Members of a pipeline group must use
  ...
  tasks. See [Pipeline Groups](ts_pipeline_groups.md#rts-pipeline-groups).
...
`domain_loop` body (see [Schedules](ts_schedules.md#rts-schedules) for what they do).
...
mismatch is checked for every pipeline type; see [Pipeline Types](ts_pipelines.md#rts-pipelines) for the
...
responsibility; this limitation is also noted in [Verification Gaps](ts_validation.md#verification-gaps).
...
[Pipeline Types](ts_pipelines.md#rts-pipelines)). It cannot check the count against the instructions the
