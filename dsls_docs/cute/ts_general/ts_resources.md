...
input) has no barriers. See [Pipeline Types](ts_pipelines.md#rts-pipelines) for the pipeline types and how
...
consumers with `wait` / `release` in the schedule (see [Schedules](ts_schedules.md#rts-schedules)).
...
aliasing are covered in [Allocators](ts_allocators.md#rts-allocators).
...
that value explicitly; see [Compile-Time Work Arguments](ts_resources.md#rts-constexpr-work-args) below.
...
one step per call (the **call-idx** column in [Reading the Printed Output](ts_printout.md#rts-printout) numbers them in
order).
