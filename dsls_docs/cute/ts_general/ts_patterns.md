...
when the simpler shapes in [Schedules](ts_schedules.md#rts-schedules) do not fit. The structural rules
that TS enforces for these patterns are listed in [Validation](ts_validation.md#rts-validation).
...
- If the wait and the release need different CTA signaling (e.g. wait on all CTAs
  ...
  `consumer_signaling_threads` (see [Pipeline Types](ts_pipelines.md#rts-pipelines)).
...
split across the lanes (see `interleave_stride` in [Pipeline Types](ts_pipelines.md#rts-pipelines) for
the lane arithmetic).
...
the verification gaps in [Validation](ts_validation.md#rts-validation)).
