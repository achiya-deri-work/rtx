...
- **Compile once, enable cross-compilation**: Write kernels in Python and cross-compile them for multiple GPU architectures, and cross-compile the host object for other CPU architectures (see [Host Cross-Compilation for AArch64](ahead_of_time_compilation.md#dsl-aot-host-cross-compilation)).
...
This guide will focus on the CuTe ABI AOT. For the Apache TVM FFI AOT, please refer to the section “Exporting Compiled Module” in [TVM FFI Compilation](tvm_ffi_compilation.md).
...
- **CUDA runtime version must match.** The exported object depends on the CUDA runtime; the target’s CUDA runtime/toolkit version must match the one CuTe DSL was built against (see [Object File Compatibility Issues](ahead_of_time_compilation.md#dsl-aot-object-compat)).
...
For more information, see the section “Exporting Compiled Module” in [TVM FFI Compilation](tvm_ffi_compilation.md).
