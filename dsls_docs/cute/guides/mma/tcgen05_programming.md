...
- [Setting up the TiledMMA, MMA Ops](tcgen05_programming.md#setting-up-the-tiledmma-mma-ops)
  ...
  - [Creating a tcgen05 MMA Op](tcgen05_programming.md#creating-a-tcgen05-mma-op)
  - [Creating a Tiled MMA](tcgen05_programming.md#creating-a-tiled-mma)
  - [Spatial tiling with a repeat count (using `atom_layout_mnk`)](tcgen05_programming.md#spatial-tiling-with-a-repeat-count-using-atom-layout-mnk)
  - [Custom tile permutation with `permutation_mnk`](tcgen05_programming.md#custom-tile-permutation-with-permutation-mnk)
  - [Creating Trivial Tiled MMA](tcgen05_programming.md#creating-trivial-tiled-mma)
- [Partitioning Tensors](tcgen05_programming.md#partitioning-tensors)

  - [Trivial TiledMma with CtaGroup.ONE MMAs (single CTA):](tcgen05_programming.md#trivial-tiledmma-with-ctagroup-one-mmas-single-cta)
  - [Trivial TiledMma with CtaGroup.TWO MMAs (2-CTA cluster, each CTA owns half the M-tile):](tcgen05_programming.md#trivial-tiledmma-with-ctagroup-two-mmas-2-cta-cluster-each-cta-owns-half-the-m-tile)
  - [Pre and Post-Conditions for TiledMMA Partitioning](tcgen05_programming.md#pre-and-post-conditions-for-tiledmma-partitioning)
  - [What happens when we use `atom_layout_mnk`?](tcgen05_programming.md#what-happens-when-we-use-atom-layout-mnk)
- [Making Fragments](tcgen05_programming.md#making-fragments)

  - [Creating fragment descriptors and descriptor tensors](tcgen05_programming.md#creating-fragment-descriptors-and-descriptor-tensors)
  - [Creating SMEM layouts for A and B](tcgen05_programming.md#creating-smem-layouts-for-a-and-b)
...
- [Beyond Simple Dense MMAs](tcgen05_programming.md#beyond-simple-dense-mmas)
  ...
  - [Block-scaled MMA](tcgen05_programming.md#block-scaled-mma)

## [Global Memory (GMEM) to MMA data flow overview](tcgen05_programming.md#id2)
...
- **Accumulator C/D**:
  ...
  - **Output flow** (GMEM destination): The load from TMEM loads results into registers and a final store writes them
...
└────────┘                    ▼         └─────────┘                   ▼                  ▼        │
                        ...
                        └───────┘     |                                                           │
                                ...
                                tTR_rAcc = make_rmem_tensor(...)
                                      │
                                      ▼
                                Load from TMEM: copy(t2r, tTR_tAcc, tTR_rAcc)
                                [TMEM → RMEM]
                                      │
                                      ▼
...
## [Setting up the TiledMMA, MMA Ops](tcgen05_programming.md#id3)
...
### [Creating a tcgen05 MMA Op](tcgen05_programming.md#id4)
...
### [Creating a Tiled MMA](tcgen05_programming.md#id5)
...
### [Spatial tiling with a repeat count (using `atom_layout_mnk`)](tcgen05_programming.md#id6)
...
### [Custom tile permutation with `permutation_mnk`](tcgen05_programming.md#id7)
...
### [Creating Trivial Tiled MMA](tcgen05_programming.md#id8)
...
## [Partitioning Tensors](tcgen05_programming.md#id9)
...
See [Making Fragments](tcgen05_programming.md#making-fragments) for more details.

### [Trivial TiledMma with CtaGroup.ONE MMAs (single CTA):](tcgen05_programming.md#id10)

For the trivial tiled MMAs with CtaGroup.ONE tcgen05 MMA operations, partitioning the mma\_tiler sized tile
is an identity operation, i.e., single CTAs’ tile is the same as the mma\_tiler sized tile.
...
`CtaGroup.ONE`, F16 atom = 128x256x16 (inst\_M=128, inst\_N=256, inst\_K=16).
...
BM and BN match the atom, BK is split into MMA\_K atom-sized steps:
...
- `tCgA: (MMA, MMA_M, MMA_K, k) = (MMA, 1, 4, 6)` — MMA\_M = BM/inst\_M = 128/128 = 1, MMA\_K = BK/inst\_K = 64/16 = 4
- `tCgB: (MMA, MMA_N, MMA_K, k) = (MMA, 1, 4, 6)` — MMA\_N = BN/inst\_N = 256/256 = 1, MMA\_K = BK/inst\_K = 64/16 = 4
- `tCgC: (MMA, MMA_M, MMA_N) = (MMA, 1, 1)` — MMA\_M = BM/inst\_M = 1, MMA\_N = BN/inst\_N = 1
...
### [Trivial TiledMma with CtaGroup.TWO MMAs (2-CTA cluster, each CTA owns half the M-tile):](tcgen05_programming.md#id11)
...
`CtaGroup.TWO`, F16 MMA atom = 128x256x16 (inst\_M=128, inst\_N=256, inst\_K=16).
...
- `tCgA: (MMA, MMA_M, MMA_K, k) = (MMA, 1, 4, 6)` — MMA\_M = (BM/2)/inst\_M = 128/128 = 1, MMA\_K = BK/inst\_K = 64/16 = 4
- `tCgB: (MMA, MMA_N, MMA_K, k) = (MMA, 1, 4, 6)` — MMA\_N = BN/inst\_N = 256/256 = 1, MMA\_K = BK/inst\_K = 64/16 = 4
- `tCgC: (MMA, MMA_M, MMA_N) = (MMA, 1, 1)` — MMA\_M = (BM/2)/inst\_M = 1, MMA\_N = BN/inst\_N = 1
...
### [Pre and Post-Conditions for TiledMMA Partitioning](tcgen05_programming.md#id12)
...
- The output of the partition will have the layout that is compatible with the MMA atom’s operand:
  - For A, the output will have the layout (MMA, MMA\_M, MMA\_K, …).
  - For B, the output will have the layout (MMA, MMA\_N, MMA\_K, …).
  - For C, the output will have the layout (MMA, MMA\_M, MMA\_N, …).
...
### [What happens when we use `atom_layout_mnk`?](tcgen05_programming.md#id13)
...
## [Making Fragments](tcgen05_programming.md#id14)
...
### [Creating fragment descriptors and descriptor tensors](tcgen05_programming.md#id15)
...
### [Creating SMEM layouts for A and B](tcgen05_programming.md#id16)
...
**Creating SMEM layouts without utilities**
...
## [Executing the GEMM (Main Loop)](tcgen05_programming.md#id17)
...
## [Reading the accumulator from TMEM](tcgen05_programming.md#id18)
...
## [Complete Workflow](tcgen05_programming.md#id19)
...
## [Beyond Simple Dense MMAs](tcgen05_programming.md#id20)

The tcgen05 MMA DSL supports more complex MMA operations than just the simple dense MMA.

- Block-scaled MMA

### [Block-scaled MMA](tcgen05_programming.md#id21)
