...
[Setting up the TiledMMA, MMA Ops](wgmma_programming.md#setting-up-the-tiledmma-mma-ops) for their full constructor parameters,
...
- [Setting up the TiledMMA, MMA Ops](wgmma_programming.md#setting-up-the-tiledmma-mma-ops)
  ...
  - [Creating a WGMMA Op](wgmma_programming.md#creating-a-wgmma-op)
  - [Creating a Tiled MMA](wgmma_programming.md#creating-a-tiled-mma)
  - [Spatial tiling with a repeat count](wgmma_programming.md#spatial-tiling-with-a-repeat-count)
  - [Custom tile permutation with `permutation_mnk`](wgmma_programming.md#custom-tile-permutation-with-permutation-mnk)
...
- [Making Fragments](wgmma_programming.md#making-fragments)
  ...
  - [Creating fragment descriptors and accumulator fragments](wgmma_programming.md#creating-fragment-descriptors-and-accumulator-fragments)
...
## [Global Memory (GMEM) to MMA data flow overview](wgmma_programming.md#id2)
...
- cta\_tiler = (BM, BN, BK) = CTA-wide tiler dimensions
...
- `tCsX` = MMA-partitioned SMEM view, e.g., (MMA, MMA\_M, MMA\_K, PIPE) for A
- `tCrX` = SMEM descriptor fragment, e.g., (MMA, MMA\_M, MMA\_K, PIPE) for A
- `acc` = RMEM accumulator, (MMA, MMA\_M, MMA\_N)
- `tCgC` = MMA-partitioned GMEM, (MMA, MMA\_M, MMA\_N)
...
- MMA = warpgroup atom thread-value layout; MMA\_M/MMA\_N/MMA\_K = repeat counts
  (e.g., BM/inst\_M), k = outer K-tiles, PIPE = pipeline stages

## [Setting up the TiledMMA, MMA Ops](wgmma_programming.md#id3)
...
### [Creating a WGMMA Op](wgmma_programming.md#id4)
...
### [Creating a Tiled MMA](wgmma_programming.md#id5)
...
### [Spatial tiling with a repeat count](wgmma_programming.md#id6)
...
### [Custom tile permutation with `permutation_mnk`](wgmma_programming.md#id7)
...
**Why WGMMA typically does not need permutation\_mnk:** The WGMMA
...
## [Partitioning Tensors](wgmma_programming.md#id8)
...
- `tCsA: (MMA, MMA_M, MMA_K, PIPE) = (MMA, 1, 4, 4)` — MMA\_M = BM / (atom\_M x inst\_M) = 128 / (2x64) = 1, MMA\_K = BK / inst\_K = 64 / 16 = 4
- `tCsB: (MMA, MMA_N, MMA_K, PIPE) = (MMA, 1, 4, 4)` — MMA\_N = BN / (atom\_N x inst\_N) = 256 / (1x256) = 1, MMA\_K = 4
- `tCgC: (MMA, MMA_M, MMA_N) = (MMA, 1, 1)` — MMA\_M = 1, MMA\_N = 1
...
Because the WGMMA instruction shape is large (64 x {64..256}),
the tiled MMA footprint typically covers the entire CTA tile in M and N
with just one or two warpgroups. This means MMA\_M and MMA\_N are often 1.
The MMA\_K dimension is where the repeat count is non-trivial (BK / inst\_K
...
- `tCsA: (MMA, MMA_M, MMA_K, PIPE) = (MMA, 2, 4, 4)` — MMA\_M = 128 / (1x64) = 2
- `tCsB: (MMA, MMA_N, MMA_K, PIPE) = (MMA, 1, 4, 4)` — MMA\_N = 128 / (1x128) = 1
...
## [Pre and Post-Conditions for Partitioning](wgmma_programming.md#id9)
...
## [Making Fragments](wgmma_programming.md#id10)
...
### [Creating fragment descriptors and accumulator fragments](wgmma_programming.md#id11)
...
Continuing the 2-warpgroup example from [Partitioning Tensors](wgmma_programming.md#partitioning-tensors)
...
WGMMA fragments for A and B are SMEM descriptors — the hardware
...
## [Creating SMEM layouts for A and B](wgmma_programming.md#id12)
...
(see [Making Fragments](wgmma_programming.md#making-fragments)).

Note

If you need finer control, you can build layout atoms directly with
`cute.nvgpu.warpgroup.make_smem_layout_atom(...)` and compose the final
SMEM layout manually via `cute.tile_to_shape`.

## [Executing the GEMM (Main Loop)](wgmma_programming.md#id13)
...
## [Complete Workflow](wgmma_programming.md#id14)
