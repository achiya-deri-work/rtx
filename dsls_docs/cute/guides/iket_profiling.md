...
[![Conceptual IKET timeline showing producer and consumer warp ranges](../../../../_images/iket_concept_timeline.svg)](../../../../_images/iket_concept_timeline.svg)

Figure 3 Conceptual view of IKET ranges and markers inside a kernel. `run-iket`
...
[![Cropped IKET timeline in Perfetto UI](../../../../_images/fp16_gemm_4_iket_quickstart.png)](../../../../_images/fp16_gemm_4_iket_quickstart.png)

Figure 4 Cropped Perfetto view from the GEMM tutorial, showing nested IKET ranges
...
A runnable example of both activation paths is available at
`examples/python/CuTeDSL/dsl_tutorials/iket_compile_options.py`.
...
| `--enabled-cluster X,Y,Z` | Dump timing for only the cluster (or thread block) at grid coordinate `X,Y,Z` to cut data volume (see [Dumping a Single Cluster or Block](iket_profiling.md#dumping-a-single-cluster-or-block)). |

### Dumping a Single Cluster or Block

For large grids you often only need timing from one place. `--enabled-cluster
X,Y,Z` dumps IKET data for just the CUDA cluster at coordinate `X,Y,Z` in the
regular cluster grid; a non-cluster kernel counts as a `1x1x1` cluster, so the
coordinate then selects a single thread block. Out-of-range coordinates fall
back to `0,0,0`.

Restricting the dump this way significantly reduces the amount of collected data
and the time spent post-processing it. It is especially helpful for workloads
that launch many thread blocks or clusters which behave similarly to one another
and each record many events: a single cluster is then representative of the whole
grid, so dumping just one keeps the useful timing while avoiding the very large
traces and slow post-processing that a full-grid dump would produce.

```bash
run-iket --output-dir ./iket_output --clobber \
    profile --enabled-cluster 0,0,0 --postprocess all \
    -- python my_kernel.py
```

To pick a different cluster per kernel, pass `--enabled-cluster-config
FILE.json` instead — a JSON object mapping a kernel-name pattern to
`[x, y, z]`, `null`, or `"all"` (`null` and `"all"` dump the full grid
for matching kernels). Patterns match as substrings of the mangled kernel name;
the exact or longest match wins, and kernels matching no entry fall back to
`--enabled-cluster` if it was given.

```json
{
  "my_gemm_kernel": [0, 0, 0],
  "epilogue": "all"
}
```
...
[![Example IKET timeline in Perfetto UI](../../../../_images/fp16_gemm_4_iket_instrument.png)](../../../../_images/fp16_gemm_4_iket_instrument.png)

Figure 5 Example IKET timeline in Perfetto UI. The exact tracks and event names
