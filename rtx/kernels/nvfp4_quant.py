...
class NVFP4RegionDelayedDualQuantKernel(NVFP4JITRegionDualQuantKernel):
    """Single-pass local delayed scaling for steady-state execution.

    Every warp owns one bounded row region.  It quantizes with the region's
    prior outer scale while accumulating the current amax from the BF16 values
    already loaded by quantization.  The successor scale is therefore emitted
    without JIT's separate observation pass or delayed tensor scaling's global
    atomics.  Current and successor arrays are swapped by the caller.
    """
    ...
    def __call__(
        ...
        sw: cute.Tensor,
        region_scales: cute.Tensor,
        next_region_scales: cute.Tensor,
        stream: cuda.CUstream,
    ):
        self.region_delayed_dual_kernel(
            x,
            weight,
            qx,
            qw,
    ...
    @cute.jit
    def _quantize_observe_region_warp(
        self,
        src: cute.Tensor,
        packed: cute.Tensor,
        scales: cute.Tensor,
        ...
        for task_group in cutlass.range(
            ...
            if local_block < region_blocks:
                ...
                observed_amax = cute.arch.fmax(
                    observed_amax, block_amax, nan=True
                )
        return nvvm.redux_sync(
            observed_amax.bitcast(Int32),
            nvvm.ReductionKind.UMAX,
            Int32(0xFFFFFFFF),
        ).bitcast(Float32)

    @cute.kernel
    def region_delayed_dual_kernel(
        self,
        x: cute.Tensor,
        weight: cute.Tensor,
        qx: cute.Tensor,
        ...
        for region_task in cutlass.range(
            ...
            prior_scale = Float32(region_scales[scale_index])
            observed = Float32(0.0)
            if is_x:
                row_begin = local_region * self.x_region_rows
                row_end = cutlass.min(row_begin + self.x_region_rows, self.x_rows)
                observed = self._quantize_observe_region_warp(
                    x, qx, sx, row_begin, row_end, prior_scale
                )
            else:
                row_begin = local_region * self.weight_region_rows
                row_end = cutlass.min(
                ...
                observed = self._quantize_observe_region_warp(
                    weight, qw, sw, row_begin, row_end, prior_scale
                )
            if lane_idx == 0:
                next_region_scales[scale_index] = self._tensor_scale_from_amax(
                    observed, self.tensor_scale_mode
                )


class NVFP4RegionDelayedBlockDualQuantKernel(NVFP4DualQuantKernel):
    """Block-parallel regional delayed quantizer.

    Unlike the one-warp-per-region experimental schedule above, this mirrors
    the bandwidth-efficient ordinary delayed grid. Each warp owns one aligned
    block group, quantizes it with the prior regional scale, and contributes a
    single monotonic atomic scale update for that region.
    """
    ...
    def __call__(
        ...
        self.region_delayed_block_kernel(
            x, weight, qx, qw, sx, sw, region_scales, next_region_scales
        ).launch(
            grid=(self.grid_ctas, 1, 1),
            block=(self.config.num_warps * 32, 1, 1),
            stream=stream,
    ...
    @cute.kernel
    def region_delayed_block_kernel(
        self,
        x: cute.Tensor,
        weight: cute.Tensor,
        qx: cute.Tensor,
        ...
        lane_in_scale = lane_idx % cfg.threads_per_scale
        blocks_per_row = self.storage_k // NVFP4_SF_VEC_SIZE
        groups_per_row = cute.ceil_div(blocks_per_row, cfg.blocks_per_warp)
        x_groups = self.x_rows * groups_per_row
        total_groups = (self.x_rows + self.weight_rows) * groups_per_row
        for task_group in cutlass.range(
            warp_linear, total_groups, warp_stride, unroll=1
        ):
            is_x = task_group < x_groups
            local_group = task_group
            scale_base = Int32(0)
            region_rows = self.x_region_rows
            if not is_x:
                local_group = task_group - x_groups
                scale_base = self.x_regions
                region_rows = self.weight_region_rows
            row = local_group // groups_per_row
            group_in_row = local_group % groups_per_row
            scale_block = group_in_row * cfg.blocks_per_warp + block_in_warp
            scale_index = scale_base + row // region_rows
            prior_scale = Float32(region_scales[scale_index])
            observed = Float32(0.0)
            if scale_block < blocks_per_row:
                if is_x:
                    observed = self._quantize_task(
                        x, qx, sx, prior_scale, row, scale_block, lane_in_scale
                    )
                else:
                    observed = self._quantize_task(
                        weight,
            ...
            observed = nvvm.redux_sync(
                observed.bitcast(Int32),
                nvvm.ReductionKind.UMAX,
                Int32(0xFFFFFFFF),
            ).bitcast(Float32)
            if lane_idx == 0:
                successor = self._tensor_scale_from_amax(
                    observed, self.tensor_scale_mode
                )
                cute.arch.atomic_fmax(
                    next_region_scales.iterator
                    + next_region_scales.layout(scale_index),
                    successor,
                    sign_bit=False,
                    scope="gpu",
                )
...
class NVFP4RegionDelayedCTADualQuantKernel(NVFP4JITRegionDualQuantKernel):
    """CTA-owned one-pass regional delayed quantization without atomics."""
    ...
    def __call__(
        ...
        self.shared_storage = SharedStorage
        self.region_delayed_cta_kernel(
            x, weight, qx, qw, sx, sw, region_scales, next_region_scales
        ).launch(
            grid=(self.grid_ctas, 1, 1),
            block=(self.config.num_warps * 32, 1, 1),
    ...
    @cute.jit
    def _quantize_observe_region_cta(
        self,
        src: cute.Tensor,
        packed: cute.Tensor,
        scales: cute.Tensor,
        ...
        for task_group in cutlass.range(
            ...
            if local_block < region_blocks:
                ...
                observed = cute.arch.fmax(observed, block_amax, nan=True)
        observed = nvvm.redux_sync(
            observed.bitcast(Int32),
            nvvm.ReductionKind.UMAX,
            Int32(0xFFFFFFFF),
        ...
        if lane_idx == 0:
            warp_amax[warp_idx] = observed
        cute.arch.sync_threads()
        cta_amax = Float32(0.0)
        if warp_idx == 0:
            if lane_idx < cfg.num_warps:
                cta_amax = Float32(warp_amax[lane_idx])
            cta_amax = nvvm.redux_sync(
    ...
    def region_delayed_cta_kernel(
        ...
        for region_task in cutlass.range(
            ...
            if is_x:
                row_begin = local_region * self.x_region_rows
                row_end = cutlass.min(row_begin + self.x_region_rows, self.x_rows)
                cta_amax = self._quantize_observe_region_cta(
                    x, qx, sx, row_begin, row_end, prior_scale, warp_amax
                )
            else:
                row_begin = local_region * self.weight_region_rows
                row_end = cutlass.min(
                ...
                cta_amax = self._quantize_observe_region_cta(
                    weight, qw, sw, row_begin, row_end, prior_scale, warp_amax
                )
            if warp_idx == 0 and lane_idx == 0:
                next_region_scales[scale_index] = self._tensor_scale_from_amax(
                    cta_amax, self.tensor_scale_mode
                )
            cute.arch.sync_threads()
...
@lru_cache(maxsize=None)
def compile_nvfp4_region_delayed_dual_quant(
    x_rows: int,
    weight_rows: int,
    k: int,
    config: NVFP4QuantConfig = NVFP4QuantConfig(),
    ...
    """Compile the one-pass steady-state local delayed quantizer."""

    kernel = NVFP4RegionDelayedCTADualQuantKernel(
        x_rows,
        weight_rows,
        k,
        config,
...
__all__ = [
    ...
    "NVFP4RegionDelayedDualQuantKernel",
    "NVFP4RegionDelayedCTADualQuantKernel",
    "NVFP4QuantConfig",
    "NVFP4QuantKernel",
