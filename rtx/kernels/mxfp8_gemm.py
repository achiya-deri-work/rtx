...
from cutlass import BFloat16, Float8E4M3FN, Float8E8M0FNU, Float32, Uint8
...
class MXFP8GemmKernel:
    def __init__(self, problem: MXFP8Problem, config: MXFP8GemmConfig):
        ...
        self.scale_barrier = pipeline.NamedBarrier(
            barrier_id=1, num_threads=config.num_mma_warps * 32
        )
    ...
    def __call__(
        ...
        class SharedStorage:
            ...
            sfa: cute.struct.Align[
                cute.struct.MemRange[Float8E8M0FNU, cute.cosize(self.sfa_layout)],
                128,
            ]
    ...
    def kernel(
        ...
        a_scale_count = cfg.tile_m * scale_blocks_per_tile
        total_scale_count = (cfg.tile_m + cfg.tile_n) * scale_blocks_per_tile
        for k_tile in cutlass.range(num_k_tiles, unroll=1):
            ...
            if warp_idx < cfg.num_mma_warps:
                ...
                for scale_task in cutlass.range(
                    ...
                    row = local_task // scale_blocks_per_tile
                    k_block = local_task % scale_blocks_per_tile
                    global_row = (
