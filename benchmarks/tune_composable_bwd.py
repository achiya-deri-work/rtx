"""Run the composable learned/global + local tuner on MXFP8 backward."""
...
from rtx.bwd_autotune import bwd_config_from_dict, update_bwd_config
...
def _legacy_best(path: Path):
    ...
    def outcome(trial):
        return trial.get("outcome", trial)

    successful = [
        trial
        for trial in trials.values()
        if outcome(trial).get("status") == "ok"
        and outcome(trial).get("median_ms") is not None
    ]
    if not successful:
        raise RuntimeError(f"legacy database {path} has no successful trials")
    best = min(successful, key=lambda trial: float(outcome(trial)["median_ms"]))
    return bwd_config_from_dict(best["config"])
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--model-pool", type=int, default=8192)
    parser.add_argument("--model-refit", type=int, default=16)
    parser.add_argument("--model-estimators", type=int, default=28)
    parser.add_argument("--model-ensembles", type=int, default=3)
    parser.add_argument("--model-features", type=int, default=96)
    ...
    parser.add_argument("--target-batch-ms", type=float, default=12.0)
    parser.add_argument("--seed", type=int, default=20260815)
    parser.add_argument("--protocol-tag", default="calibrated_bwd_batch_v1")
    parser.add_argument(
        "--orchestration", choices=("sequential", "bandit"), default="sequential"
    )
    parser.add_argument(
        "--initial-quant-launches",
        choices=("separate", "dual"),
        default="dual",
    )
...
def main() -> None:
    ...
    harness = BwdBenchmarkHarness(shape, protocol, seed=args.seed)
    ...
    if args.initial_config is not None:
        ...
        initial = bwd_config_from_dict(initial_document["config"])
    else:
        initial = (
            DEFAULT_MXFP8_BWD_CONFIG
            if args.legacy_db is None
            else _legacy_best(args.legacy_db)
        )
    initial = update_bwd_config(
        initial,
        {
            "dx": {"quant_launches": args.initial_quant_launches},
            "dw": {"quant_launches": args.initial_quant_launches},
        },
    )
