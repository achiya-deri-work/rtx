...
manifest="autotune_manifests/blackwell_diversity_atlas_v1.json"
campaign="blackwell_diversity_atlas_v1"
...
worker() {
  ...
  local reserve_seconds="${RTX_AUTOTUNE_FINALIZE_RESERVE_SECONDS:-300}"
  local deadline_epoch=$(( $(date +%s) + wall_seconds ))
  local tuning_deadline=$(( deadline_epoch - reserve_seconds ))
  ...
  if [[ -n "${RTX_AUTOTUNE_SHARD_INDEX:-}" || -n "${RTX_AUTOTUNE_SHARD_COUNT:-}" ]]; then
    if [[ -z "${RTX_AUTOTUNE_SHARD_INDEX:-}" || -z "${RTX_AUTOTUNE_SHARD_COUNT:-}" ]]; then
      echo "RTX_AUTOTUNE_SHARD_INDEX and RTX_AUTOTUNE_SHARD_COUNT must be set together" >&2
      return 2
    fi
    shard_args=(--shard-index "$RTX_AUTOTUNE_SHARD_INDEX" --shard-count "$RTX_AUTOTUNE_SHARD_COUNT")
  fi

  echo "START campaign=$campaign duration=$duration output=$output_root"
  echo "COMMIT $(git rev-parse HEAD)"
  echo "POLICY coverage-first milestones=${RTX_AUTOTUNE_MILESTONES:-1,2,4,8,16,32,64,128}"
  ...
  while true; do
    local remaining=$(( tuning_deadline - $(date +%s) ))
    if (( remaining <= 0 )); then
      echo "DEADLINE tuning budget exhausted; reserving ${reserve_seconds}s for audit/collection"
      break
    fi
    attempt=$((attempt + 1))
    ...
    "$python_bin" -m rtx.autotune.dataset run "$manifest" \
      ...
      --context-slice "${RTX_AUTOTUNE_CONTEXT_SLICE:-45s}" \
      --trial-milestones "${RTX_AUTOTUNE_MILESTONES:-1,2,4,8,16,32,64,128}" \
      --initial-promote "${RTX_AUTOTUNE_INITIAL_PROMOTE:-1}" \
      --strategy-orchestration bandit \
      --strategy-bandit-exploration "${RTX_AUTOTUNE_STRATEGY_EXPLORATION:-0.45}" \
...
nohup "$repo_root/benchmarks/launch_diversity_atlas.sh" \
