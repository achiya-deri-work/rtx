...
manifest="autotune_manifests/nvfp4_revision8_release_v1.json"
campaign="nvfp4_revision8_release_v1"
...
worker() {
  ...
  local report_root="$3"
  local calibration="$4"
  local deadline_epoch=$(( $(date +%s) + $(duration_to_seconds "$duration") ))
  ...
  while true; do
    ...
    "$python_bin" -m rtx.autotune.dataset run "$manifest" \
      ...
      --trial-milestones "${RTX_AUTOTUNE_MILESTONES:-8,16,32,64,128,256}" \
  ...
  "$python_bin" -m rtx.autotune.dataset verify-winners "$manifest" \
    --device cuda:0 --output-dir "$output_root" --promote 5
...
nohup "$repo_root/benchmarks/launch_nvfp4_revision8_release.sh" \
